...
/*   Updated: 2023/09/27 23:35:47 by hyungdki         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	int ao_idx, int pp_idx)
{
	...
	if (data->pipe_cnt[ao_idx] > 1
		...
		child_exit(data, tmp, 1, "redirection error");
	result = sign_redirection(data, cdata->tkns);
	if (result == 1)
		exit(1);
	else if (result == 2)
		child_exit(data, tmp, 1, "file open error");
	else if (result == 3)
		child_exit(data, tmp, 1, "redirection error");
	else if (result == 4)
		child_exit(data, tmp, 1, "stat() error");
}
...
static void	child_execute_cmd1(t_data *data, t_cdata *cdata)
{
	if (cdata->cmd[0] == '\0')
	{
		...
		child_exit(data, cdata->pipe_cnt, 127, T_NULL);
	}
...
static void	child_execute_cmd2(t_data *data, t_cdata *cdata)
{
	...
	if (execve(cdata->cmd_path, cdata->argu_lst, cdata->envp_lst) == -1)
	{
		...
		child_exit(data, cdata->pipe_cnt, 1, T_NULL);
	}
}
