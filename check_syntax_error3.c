...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/09/23 19:22:18 by junehyle          #+#    #+#             */
/*   Updated: 2023/09/23 19:22:24 by junehyle         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_bool	parentheses_in_parentheses(char *cmd, int *idx)
{
	...
	while (cmd[--idx2] != '(' && cmd[idx2] != '|' && cmd[idx2] != '&')
	{
		...
		{
			if (ft_isblank(cmd[idx2]) == FALSE && (ft_isblank(cmd[idx2 + 1])
					== '(' || ft_isblank(cmd[idx2 + 1]) == TRUE))
				word_cnt++;
			if (word_cnt >= 2)
				return (syntax_error_print("("));
		}
...
t_bool	check_parentheses_syntax(char *cmd)
{
	...
	while (cmd[++idx] != '\0')
	{
		...
		else if (parentheses_flag == FALSE
			&& (cmd[idx] == '\"' || cmd[idx] == '\''))
			ignore_quote(cmd, &idx);
		else if (cmd[idx] == ')')
			return (syntax_error_print(")"));
	...
	while (cmd[++idx] != '\0')
	{
		if (cmd[idx] == '$' && cmd[idx + 1] == '{')
		{
			...
			while (cmd[++idx] != '}')
			{
				...
				{
					write(2,
						"minishell: syntax error, unclosed brace\n", 41);
					return (FALSE);
				}
			}
