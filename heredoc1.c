...
/*   Updated: 2023/09/25 19:46:05 by hyungdki         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_bool	heredoc_make1_1_2(t_dll *dll, int *idx, char *del,
					char *make_path);
static char		*heredoc_make1_2_2(t_dll *dll, int *idx, char *del,
					char *make_path);
...
t_bool	heredoc_make1_1(t_dll *dll, int *idx, char *del)
{
	...
	if (heredoc_make1_1_2(dll, idx, del, make_path) == FALSE)
		return (FALSE);
	return (TRUE);
}
...
t_bool	heredoc_make1_2(t_dll *dll, t_dllnode *ptr, int *idx, char *del)
{
	...
	name = heredoc_make1_2_2(dll, idx, del, make_path);
	if (name == T_NULL)
		return (FALSE);
